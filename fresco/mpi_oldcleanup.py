...
for folder in tqdm(folder_names):
    ...
    if not os.path.isfile(folder+'/pairs_cleanup_'+pickle_index+'_mpi_'+str(mpi_rank)+'.pickle'):
            if os.path.isfile(folder+'/mols'+pickle_index+'.pickle'):
                with open(folder+'/mols'+pickle_index+'.pickle', 'rb') as handle:
